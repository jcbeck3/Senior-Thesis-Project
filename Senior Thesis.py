...
print("Number of trials in a block: " + str(trial_by_block))
            ...
            if stimnum != 0:
                for i in range(stimnum):
                    ...
                    if (diagnostic_word.status == STARTED and t >= frameRemains) or (diagnostic_word.status == STARTED and (len(key_list)>0)):
                        ...
                        thisExp.addData('WordCongruency', congruency[stimmatrix[1][i]])
                        thisExp.addData('Word Orientation', orientation[stimmatrix[0][i]])
